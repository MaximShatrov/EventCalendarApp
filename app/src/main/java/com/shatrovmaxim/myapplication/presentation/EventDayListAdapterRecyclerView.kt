...
import com.shatrovmaxim.myapplication.utils.EventDayList
import com.shatrovmaxim.myapplication.R
import com.shatrovmaxim.myapplication.repository.entity.EventEntity
import java.text.SimpleDateFormat
import java.util.*

class EventDayListAdapterRecyclerView(
    private val events: EventDayList,
    private val listener: OpenViewListener
) :
    RecyclerView.Adapter<EventDayListAdapterRecyclerView.ViewHolder>() {

    override fun getItemCount() = events.getSize()
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        holder.initCard(position, events.getEvent(position), listener)
        // обработка нажатия
        /*
    ...
    class ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun initCard(position: Int, event: EventEntity, listener: OpenViewListener) {
            ...
            if ((event.date_start.time == 0L) && (event.date_finish.time == 0L)) {
            ...
            eventTime.text = simpleDateFormat.format(Date(event.date_start.getTime())) + " - " + simpleDateFormat.format(Date(event.date_finish.getTime()))
